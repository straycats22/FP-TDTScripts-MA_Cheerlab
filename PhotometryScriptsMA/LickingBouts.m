...
data = TDTbin2mat('C:\Users\migue\OneDrive - upf.edu\Documentos\MATLAB\TDTMatlabSDK\Data\GrabDA Sucrose Pellet\DA4_PR2', 'TYPE', {'epocs', 'scalars', 'streams'});
...
hold on;
p1 = plot(time, data.streams.(GrabDA).data,'color',green,'LineWidth',2);
...
data.streams.(GrabDA).data = arrayfun(@(i)...
    mean(data.streams.(GrabDA).data(i:i+N-1)),...
    1:N:length(data.streams.(GrabDA).data)-N+1);
...
time = time(1:N:end);
time = time(1:length(data.streams.(GrabDA).data));
...
Rew_on = data.epocs.Rew_.onset;
Rew_off = data.epocs.Rew_.offset;
Rew_x = reshape(kron([Rew_on, Rew_off], [1, 1])', [], 1);
sz = length(Rew_on);
d = data.epocs.Rew_.data';
y_scale = 5; %adjust according to data needs
y_shift = 0; %scale and shift are just for asthetics
Rew_y = reshape([zeros(1, sz); d; d; zeros(1, sz)], 1, []);

%%
% First subplot in a series: dFF with lick epocs
figure('Position',[100, 100, 800, 400]);
p1 = plot(time, dFF, 'Color',green,'LineWidth',2); hold on;
p2 = plot(Rew_x, y_scale*(Rew_y) + y_shift,'color',cyan,'LineWidth',2);
title('Detrended, y-shifted dFF','fontsize',16);
legend([p1 p2],'GrabDA','Reward');
axis tight
...
Reward = 'Rew/';
data.epocs.Rew_.name = Reward;
data.epocs.Rew_.onset = [];
data.epocs.Rew_.offset = [];
data.epocs.Rew_.typeStr = data.epocs.Rew_.typeStr;
data.epocs.Rew_.data = [];

%%
% Find differences in onsets and threshold for major difference indices
Rew_on_diff = diff(data.epocs.Rew_.onset);
BOUT_TIME_THRESHOLD = 10; % example bout time threshold, in seconds
Rew_diff_ind = find(Rew_on_diff >= BOUT_TIME_THRESHOLD);

%%
% Make an onset/ offset array based on threshold indices

diff_ind_i = 1;
for i = 1:length(Rew_diff_ind)
    % BOUT onset is thresholded onset index of lick epoc event
    data.epocs.Rew_.onset(i) = data.epocs.Rew_.onset(diff_ind_i);
    
    % BOUT offset is thresholded offset of lick event before next onset
    data.epocs.Rew_.offset(i) = ...
        data.epocs.Rew_.offset(Rew_diff_ind(i));
    
    data.epocs.Rew_.data(i) = 1; % set the data value, arbitrary 1
    diff_ind_i = Rew_diff_ind(i) + 1; % increment the index
end

% Special case for last event to handle lick event offset indexing
data.epocs.Rew_.onset = [data.epocs.Rew_.onset, ...
   data.epocs.Rew_.onset(end)];
data.epocs.Rew_.offset = [data.epocs.Rew_.offset, ...
   data.epocs.Rew_.offset(end)];
data.epocs.Rew_.data = [data.epocs.Rew_.data, 1];

% Transpose the arrays to make them column vectors like other epocs
data.epocs.Rew_.onset = data.epocs.Rew_.onset';
data.epocs.Rew_.offset = data.epocs.Rew_.offset';
data.epocs.Rew_.data = data.epocs.Rew_.data';
...
licks_array = zeros(length(data.epocs.Rew_.onset),1);
for i = 1:length(data.epocs.Rew_.onset)
    % Find number of licks in licks_array between onset ond offset of
    % Our new lick BOUT (LICK_EVENT)
    licks_array(i) = numel(find(data.epocs.Rew_.onset >=...
        data.epocs.Rew_.onset(i) & data.epocs.Rew_.onset <=...
        data.epocs.Rew_.offset(i)));
end
...
data.epocs.Rew_.onset((licks_array < MIN_LICK_THRESH)) = [];
data.epocs.Rew_.offset((licks_array < MIN_LICK_THRESH)) = [];
data.epocs.Rew_.data((licks_array < MIN_LICK_THRESH)) = [];

%%
% Make continuous time series for lick BOUTS for plotting
LICK_EVENT_on = data.epocs.Rew_.onset;
LICK_EVENT_off = data.epocs.Rew_.offset;
...
dFF_min = min(dFF);
dFF_max = max(dFF);
for i = 1:numel(data.epocs.Rew_.onset)
    h1(i) = area([data.epocs.Rew_.onset(i) ...
        data.epocs.Rew_.offset(i)], [dFF_max dFF_max], ...
...
fs = data.streams.(GrabDA).fs/N; % recall we downsampled by N = 100 earlier
% time span for peri-event filtering, PRE and POST
TRANGE = [-1*PRE_TIME*floor(fs),POST_TIME*floor(fs)];

%%
% Pre-allocate memory
trials = numel(data.epocs.Rew_.onset);
...
for i = 1:trials
    ...
    if data.epocs.Rew_.onset(i) < PRE_TIME
        dFF_snips{i} = single(zeros(1,(TRANGE(2)-TRANGE(1))));
        continue
    else
        % Find first time index after bout onset
        array_ind(i) = find(time > data.epocs.Rew_.onset(i),1);
